...
| **RN-07: Composição do Lote Composto** | `lote_composto` / `LoteComposto` | `<aviario>-<lote>` ou `<aviario>-<lote>-<nucleo>` | O identificador `Lote Composto` (ou `LoteComposto`) é formado pela concatenação do aviário e lote (ex: `1223-8`), podendo opcionalmente incluir a identificação do núcleo (ex: `1223-8-1`). |
