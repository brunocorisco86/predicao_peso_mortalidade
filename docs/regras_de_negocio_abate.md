...
| **RN-13: Tratamento de Inversão Biométrica & Suavização Monotônica** | `flag_inversao_biometrica`, pesagens suavizadas $W_t$ | Suavização Regressão Isotônica | Identificação de erros de amostragem de campo quando o peso corporal amostrado em idade posterior é menor que em idade anterior ($W_{t+1} < W_t \cdot 0,95$). Aplica suavização monotônica isométrica não-decrescente e rebaixa a confianca amostral em inversões severas ($>15\%$). |
