...
Este documento consolida o mapeamento arquitetural do fluxo de dados, a auditoria dos scripts da esteira de ETL e o rastreamento rigoroso de todas as Regras de Negócio (RN-01 a RN-13) aplicadas na predição de peso de abate e mortalidade.
...
## 2. Mapeamento das Regras de Negócio (RN-01 a RN-13)
...
| **RN-07** | **Composição do Lote Composto** | Estruturado via concatenações (ex: `100-39`) em queries de relacionamento (chaves primárias de joins no ETL). |
